...
Ảnh của mọi lần thử được lưu trước khi phán xét (`attendance.py`: `put_private` đứng trước geofence), nên lượt bị chặn vì vị trí cũng có ảnh cho người quản lý xem. **CPU của máy chủ**: dlib chạy một luồng, nên `face-ai` tối đa dùng một nhân dù có 12 (đo: ba vòng dẫn hướng song song = 105% CPU); `OMP_NUM_THREADS=2` để OpenBLAS khỏi mở 12 luồng chờ việc, `cpus: 4` làm trần. Healthcheck 30 giây (từng là 5–10 giây và là thứ giữ VM không bao giờ rảnh). Vòng dẫn hướng camera giãn ra 2 giây khi đã đạt, dừng khi tab ẩn.
