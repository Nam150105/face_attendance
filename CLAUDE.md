...
- Camera: `CameraCapture` chỉ nói bốn điều trước khi chụp (khuôn mặt · một người · ánh sáng · độ nét) từ `POST /faces/guide` → `checks`; kết quả đúng/sai chỉ hiện sau khi máy chủ trả lời (`ResultCard`). Quyền camera/vị trí ghi nhớ ở `lib/device.ts` (localStorage + Permissions API) để lần sau camera tự mở; Safari iPhone vẫn hỏi lại theo phiên — ngoài tầm của trang web.
